...
const struct command rasterModeStartCommand =
...
inline void setPrintDensity(int printDensity,int printBreakTime)
{
#ifdef DEBUGP
    if (lfd)
        fprintf (lfd,"Set printer Density :\n printDensity = %d\n printBreakTime = %d\n",printDensity,printBreakTime);
#endif
    mputchar(0x12);       // DC2
...
void initializeSettings(char * commandLineOptionSettings)
{
    ...
    settings.sleepafter =  getOptionChoiceIndex("Sleep"          , ppd);
    settings.softcontrol =  getOptionChoiceIndex("FlowControl"          , ppd);
#ifdef DEBUGP
    if (lfd)
        fprintf (lfd,"get FlowControl : %d\n",settings.softcontrol);
#endif
...
int main(int argc, char *argv[])
{
    ...
    while (cupsRasterReadHeader2(ras, &header))
    {
        ...
        //Settings Head parameter
        setPrintSettings(header.cupsInteger[0],header.cupsInteger[1],header.cupsInteger[2]);
        setPrintDensity(header.cupsInteger[3],header.cupsInteger[4]);
        int dotdelay = (header.cupsInteger[1]* 10) + (header.cupsInteger[2] * 10) + (header.cupsInteger[4] * 250);
    ...
    if (lfd)
    {
        ...
        fprintf(lfd,"\nDots = %u\nheating time = %u\nheating interval = %u\ndensity = %u\break time =%u\n",header.cupsInteger[0],header.cupsInteger[1],header.cupsInteger[2],header.cupsInteger[3],header.cupsInteger[4]);
    }
#endif
    ...
    if (lfd) fprintf(lfd,"\nChecked %d bytes of %d for zero\n",j,rest_bytes);
            ...
            skiplines(0);
            if (settings.softcontrol == 1)
            {
                lineDelay((char*)rasterData,rest_bytes,dotdelay);
            }
        }
